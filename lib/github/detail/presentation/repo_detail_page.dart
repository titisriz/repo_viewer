  ...
  void initState() {
    ...
    Future.microtask(
      () => ref
          ...
          .getRepoDetail(widget.githubRepo.fullName),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer(
      builder: (context, ref, child) => WillPopScope(
        onWillPop: (() async {
          if (state.isStarredStatusHasChanged) {
            ref
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            actions: [
              state.maybeMap(orElse: () {
                ...
                return IconButton(
                  onPressed: () {
                    !successState.githubRepoDetail.isFresh
                        ? null
                        : ref
                            .read(repoDetailNotifierProvider.notifier)
                            .switchStarredStatus();
                  },
                  icon: Icon(!successState.githubRepoDetail.isFresh
                      ? MdiIcons.starRemoveOutline
                      : successState.githubRepoDetail.entity?.isStarred == true
                          ? Icons.star
                          : Icons.star_outline),
                  splashRadius: 20,
                );
          ...
          body: state.map(
            ...
            loadSuccess: (_) {
              if (_.githubRepoDetail.entity == null) {
                const NoResultsDisplay(
                  message: "This is all we can find on this repo",
                );
              } else {
                return WebView(
                  ...
                  initialUrl: Uri.dataFromString(
                    ...
                    <body> ${_.githubRepoDetail.entity?.html}</body>
                    </html>
                    $css
                    ''',
                    mimeType: 'text/html',
            ...
            loadFailure: (_) => Center(
              child: Text(_.githubFailure.toString()),
            ),
          ),
        ),
      ),
